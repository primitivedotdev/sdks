...
RUST_CLI_GENERATED_PATHS := openapi/primitive-api.codegen.json packages/api-core/src/api packages/api-core/src/openapi/openapi.generated.ts packages/api-core/src/openapi/operations.generated.ts cli-rust/src/operation-manifest.json
...
rust-cli-check-generated: rust-cli-manifest-bytewise
...
rust-cli-release-smoke: rust-cli-release-build
	...
	grep -q -- '"operationId": "getAccount"' "$$smoke_dir/prim-list-operations.json" && \
	if [ -n "$${RUST_CLI_CARGO_TARGET:-}" ]; then cargo install --quiet --path cli-rust --locked --root "$$smoke_dir" --target "$$RUST_CLI_CARGO_TARGET"; else cargo install --quiet --path cli-rust --locked --root "$$smoke_dir"; fi && \
	"$$smoke_dir/bin/primitive" describe sending:send-email > "$$smoke_dir/describe-send-email.json" && \
	grep -q -- '"operationId": "sendEmail"' "$$smoke_dir/describe-send-email.json"
...
cli-operation-coverage:
	node scripts/assert-cli-operation-request-coverage.mjs --require-all --min-aliases 58

cli-help-parity: cli-command-surface-parity rust-cli-build
	node scripts/run-cli-help-sweep.mjs --compare-flags --node-bin "$${NODE_CLI_BIN:-node $(CURDIR)/cli-node/bin/run.js}" --rust-bin "$${RUST_CLI_BIN:-$(CURDIR)/cli-rust/target/debug/primitive}"
...
cli-archive-parity: cli-command-surface-parity cli-operation-coverage rust-cli-release-build
	...
	archive_path=$$(node -e "const fs=require('fs'); process.stdout.write(JSON.parse(fs.readFileSync(process.argv[1], 'utf8')).archive);" "$$smoke_dir/package.json") && \
	tar -C "$$extract_dir" -xzf "$$archive_path" && \
	node scripts/run-cli-help-sweep.mjs --compare-flags --node-bin "$${NODE_CLI_BIN:-node $(CURDIR)/cli-node/bin/run.js}" --rust-bin "$$extract_dir/primitive" && \
