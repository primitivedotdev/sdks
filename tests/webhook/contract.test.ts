...
describe("contract", () => {
  describe("constants", () => {
    it("exports correct inline threshold", () => {
      expect(RAW_EMAIL_INLINE_THRESHOLD).toBe(262144); // 256 KB
    });
  ...
  describe("buildEmailReceivedEvent", () => {
    ...
    it("nulls out body_text and body_html for large emails", () => {
      ...
      // Raw should not be inlined
      expect(event.email.content.raw.included).toBe(false);

      // Parsed status should still be complete, but body content should be null
      ...
      if (event.email.parsed.status === "complete") {
        ...
        // Attachments metadata should still be present
    ...
    describe("ISO 8601 timestamp validation", () => {
      ...
      it("error message suggests correct format", () => {
        ...
        // Error should include an example of correct format
