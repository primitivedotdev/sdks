...
describe('filename sanitization', () => {
  test('removes path traversal sequences', () => {
    ...
    expect(sanitizeFilename('..\\..\\windows\\system32', 0)).toBe('____windows_system32');
  });
  ...
  test('replaces unicode with underscore (archiver PaxHeader issue)', () => {
    ...
    expect(sanitizeFilename('\u0434\u043E\u043A\u0443\u043C\u0435\u043D\u0442.docx', 0)).toBe('________.docx');
    expect(sanitizeFilename('Caf\u00E9.pdf', 0)).toBe('Caf_.pdf');
    expect(sanitizeFilename('Report \uD83D\uDCE7.pdf', 0)).toBe('Report __.pdf');
  });
  ...
  test('removes colons (archiver treats them as drive letters)', () => {
    ...
    expect(sanitizeFilename('Climate: The Report.pdf', 0)).toBe('Climate- The Report.pdf');
    expect(sanitizeFilename('C:\\users\\test.txt', 0)).toBe('C-_users_test.txt');
    expect(sanitizeFilename('Time: 10:30 AM.pdf', 0)).toBe('Time- 10-30 AM.pdf');
  });

  test('handles combined unicode and colons', () => {
    expect(sanitizeFilename('\u5831\u544A\u66F8: Final.pdf', 0)).toBe('___- Final.pdf');
    expect(sanitizeFilename('Caf\u00E9: Menu \uD83D\uDCE7.pdf', 0)).toBe('Caf_- Menu __.pdf');
  });
...
describe('content type normalization', () => {
  ...
  test('strips parameters', () => {
    ...
    expect(normalizeContentType('application/pdf; name="file.pdf"')).toBe('application/pdf');
  });
...
describe('edge cases', () => {
  ...
  test('handles quoted-printable encoding', async () => {
    ...
    expect(result.bodyText).toContain('soft line breaks which should be removed during decoding');
