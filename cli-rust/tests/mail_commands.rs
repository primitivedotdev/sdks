...
#[test]
fn send_and_reply_value_flags_reject_following_flag_tokens() {
    let send = build_send_command_plan_from_args(
        &argv(&[
            "--api-key",
            "prim_test",
            "--from",
    ...
    .expect_err("flag token should not be consumed as --body value");
    assert!(send.to_string().contains("Flag --body expects a value"));

    let reply =
        build_reply_command_plan_from_args(&argv(&["--id", "email_123", "--body", "--html-stdin"]))
            .expect_err("flag token should not be consumed as --body value");
    assert!(reply.to_string().contains("Flag --body expects a value"));
}

#[test]
fn send_rejects_no_wait_unknown_flags_and_duplicate_scalar_flags_as_usage_errors() {
    let no_wait = build_send_command_plan_from_args(
        &argv(&[
            "--from",
            "a@example.com",
            "--to",
    ...
    .expect_err("--no-wait should not be accepted");
    assert_eq!(no_wait.to_string(), "Nonexistent flag: --no-wait");

    let unknown = build_send_command_plan_from_args(
        &argv(&[
            "--from",
            "a@example.com",
            "--to",
    ...
    .expect_err("unknown send flag should fail");
    assert_eq!(unknown.to_string(), "Nonexistent flag: --wat");

    let inline_bool = build_send_command_plan_from_args(
        &argv(&[
            "--from",
            "a@example.com",
            "--to",
    ...
    .expect_err("inline boolean send flag should fail");
    assert_eq!(inline_bool.to_string(), "Unexpected argument: false");

    let duplicate = build_send_command_plan_from_args(
        &argv(&[
            "--from",
            "a@example.com",
            "--to",
    ...
    .expect_err("duplicate send value flag should fail");
    assert_eq!(
        duplicate.to_string(),
        "Flag --body can only be specified once"
    );

    let duplicate_bool = build_send_command_plan_from_args(
        &argv(&[
            "--from",
            "a@example.com",
            "--to",
    ...
    .expect("duplicate boolean send flag should be idempotent");
    assert_eq!(
        duplicate_bool.request.body,
        Some(json!({
            "from": "a@example.com",
            "to": "b@example.com",
            "subject": "Hello",
...
#[test]
fn send_value_flags_accept_single_dash_values() {
    let plan = build_send_command_plan_from_args(
        &argv(&[
            "--from",
            "a@example.com",
...
#[test]
fn plans_exact_top_level_chat_reply_parent_without_default_lookup() {
    let plan = build_chat_command_plan_from_args_with_default_lookup(
        &argv(&[
            "agent@example.com",
            "--reply",
    ...
    .expect("exact reply parent should defer send construction until runtime");
    ...
    match plan.reply_context {
        ChatReplyContextPlan::Exact { id, request } => {
            ...
            assert_eq!(request.method, "GET");
            assert_eq!(request.path, "/emails/email_123");
        }
        other => panic!("unexpected reply context {other:?}"),
    }
}
...
#[test]
fn plans_reply_command_with_attached_short_attachment_alias() {
    let (dir, attachment_path) = write_temp_attachment(
        "reply-attached-a",
        "primitive-parser-probe-note.txt",
        b"note",
    );
    let plan = build_reply_command_plan_from_args(&[
        "--id".to_string(),
        ...
        "--body".to_string(),
        "hi".to_string(),
        format!("-a{attachment_path}"),
    ])
    .expect("build reply plan with -a/path");

    assert_eq!(
        plan.request.body,
        Some(json!({
            "body_text": "hi",
            "attachments": [{"content_base64": "bm90ZQ==", "filename": "primitive-parser-probe-note.txt"}]
        }))
    );
