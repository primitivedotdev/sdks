...
fn start_single_response_json_server(
...
#[test]
fn deploy_wait_failed_prints_function_json_and_exits_one() {
    let dir = temp_dir("deploy-wait-failed");
    fs::create_dir_all(&dir).expect("create temp dir");
    let bundle = dir.join("bundle.js");
    fs::write(&bundle, "export default async function handler() {}\n").expect("write bundle");
    let (api_base_url, handle) = start_single_response_json_server(json!({
        "data": {
            "id": "fn_failed",
            "name": "triage",
    ...
    let (code, stdout, stderr) = run_function_command(
        &args(&[
            "functions",
            "deploy",
            "--name",
    ...
    let request = handle.join().expect("test server");
    let value: serde_json::Value = serde_json::from_str(&stdout).expect("function json");

    assert_eq!(code, 1);
    assert!(request.starts_with("POST /v1/functions HTTP/1.1\r\n"));
    assert_eq!(value["id"], "fn_failed");
    assert_eq!(value["deploy_status"], "failed");
    assert_eq!(
        stderr,
        "Function fn_failed deploy failed: bundle syntax error\n"
    );
    fs::remove_dir_all(dir).ok();
}

#[test]
fn redeploy_wait_failed_prints_function_json_and_exits_one() {
    let dir = temp_dir("redeploy-wait-failed");
    fs::create_dir_all(&dir).expect("create temp dir");
    let bundle = dir.join("bundle.js");
    fs::write(&bundle, "export default async function handler() {}\n").expect("write bundle");
    let (api_base_url, handle) = start_single_response_json_server(json!({
        "data": {
            ...
            "name": "triage",
            "deploy_status": "failed",
            "deploy_error": "missing default export"
        }
    }));
    ...
    assert_eq!(code, 1);
    assert!(request.starts_with("PUT /v1/functions/fn_redeploy HTTP/1.1\r\n"));
    assert_eq!(value["id"], "fn_redeploy");
    assert_eq!(value["deploy_status"], "failed");
    assert_eq!(
        stderr,
        "Function fn_redeploy deploy failed: missing default export\n"
    );
    fs::remove_dir_all(dir).ok();
}

#[test]
fn deploy_wait_timeout_exits_two() {
    let dir = temp_dir("deploy-wait-timeout");
    fs::create_dir_all(&dir).expect("create temp dir");
    let bundle = dir.join("bundle.js");
    fs::write(&bundle, "export default async function handler() {}\n").expect("write bundle");
    let (api_base_url, handle) = start_single_response_json_server(json!({
    ...
    assert!(
        stderr.contains(
            "Waiting for function fn_pending deploy to finish (current status: pending)..."
        ),
        "{stderr}"
    );
    assert!(
        stderr.contains(
            "Timed out after 1s waiting for function fn_pending deploy to finish (last status: pending)."
        ),
        "{stderr}"
    );
    fs::remove_dir_all(dir).ok();
