...
fn root_dispatch_rejects_poll_shortcut_like_node_cli() {
    ...
    assert!(spaced.to_string().contains("Command emails:poll not found"));

    let colon = friendly::dispatch(args(&["emails:poll", "--help"]))
        .expect_err("colon poll shortcut should not route");
    assert!(colon.to_string().contains("Command emails:poll not found"));
}
...
#[test]
fn generated_email_boolean_flags_follow_oclif_parse_errors() {
    let no_json = build_emails_get_request_from_args(&args(&["--id", "em_123", "--no-json"]))
        .expect_err("negated json flag should fail");
    assert!(no_json.to_string().contains("Nonexistent flag: --no-json"));

    let inline_time =
        build_emails_get_request_from_args(&args(&["--id", "em_123", "--time=false"]))
            .expect_err("inline boolean value should fail");
    assert!(inline_time
        .to_string()
        .contains("Unexpected argument: false"));
}
...
fn emails_show_is_not_a_routed_alias_or_help_surface() {
    ...
    assert!(error.to_string().contains("Command emails:show not found"));
}
