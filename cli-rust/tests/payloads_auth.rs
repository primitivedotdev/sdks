...
fn response_body_for_request(request: &str) -> Value {
    if request.starts_with("POST /oauth/token HTTP/1.1\r\n") {
        return json!({
...
#[test]
fn payloads_pull_refreshes_expired_saved_oauth_credentials() {
    ...
    let (api_base_url, handle) = start_payload_auth_server();
    {
        ...
        assert!(
            requests[0].contains("refresh_token=prim_ort_old"),
            "{}",
            requests[0]
        );
        assert!(
            requests[1].contains("\r\nauthorization: Bearer prim_oat_new\r\n"),
            "{}",
            requests[1]
        );
