...
#[test]
fn plans_zone_file_with_attached_short_output_alias() {
    let plan = build_domains_command_plan(
        "domains zone-file",
        &args(&["--id", "dom_123", "-o/tmp/primitive-parser-probe-zone.txt"]),
    )
    .expect("zone file plan with -o/path");

    assert_eq!(
        plan.selector,
        DomainZoneFileSelector::Id("dom_123".to_string())
    );
    assert_eq!(
        plan.output,
        ZoneFileOutputDestination::File("/tmp/primitive-parser-probe-zone.txt".to_string())
    );
}
...
fn selects_domain_id_from_list_response() {
    ...
    let selection = select_domain_by_name(&response, "Example.COM").expect("exact match");
    assert_eq!(selection.id, "domain_2");
    assert_eq!(selection.domain, "Example.COM");

    let case_mismatch =
        select_domain_by_name(&response, "example.com").expect_err("case mismatch should fail");
    assert!(case_mismatch
        .to_string()
        .contains("Domain example.com was not found."));
...
fn rejects_missing_or_ambiguous_zone_file_inputs() {
    ...
    .expect_err("ambiguous exact domain should fail");
