...
fn routes_add_rejects_invalid_positionals_and_target_flags() {
    ...
    let missing_function_value = build_routes_add_request_from_args(&args(&[
        "b@example.com",
        "--function",
        "--disabled",
        "--api-key",
    ...
    .expect_err("flag token should not be consumed as --function value");
    assert!(missing_function_value
        .to_string()
        .contains("Flag --function expects a value"));
