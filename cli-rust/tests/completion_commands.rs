use primitive_rust::completion_commands::{
    autocomplete_help_text, completion_command_output, completion_help_text, render_completion,
};
...
#[test]
fn completion_command_matches_node_tty_behavior() {
    let bash_redirect =
        completion_command_output("primitive", "bash", false).expect("bash redirect output");
    assert!(bash_redirect.contains("complete -F _primitive_completion primitive"));
    assert!(!bash_redirect.contains("Setup Instructions"));

    let bash_tty = completion_command_output("primitive", "bash", true).expect("bash tty output");
    assert!(bash_tty.contains("Setup Instructions"));
    assert!(bash_tty.contains("primitive autocomplete script bash"));

    let zsh_redirect =
        completion_command_output("primitive", "zsh", false).expect("zsh redirect output");
    assert!(zsh_redirect.starts_with("#compdef primitive"));
    assert!(!zsh_redirect.contains("Setup Instructions"));

    let powershell_redirect =
        completion_command_output("primitive", "powershell", false).expect("powershell output");
    assert!(powershell_redirect.contains("Setup Instructions"));

    let fish_tty = completion_command_output("primitive", "fish", true).expect("fish output");
    assert!(fish_tty.contains("__fish_primitive_needs_command"));
    assert!(!fish_tty.contains("Setup Instructions"));
}
