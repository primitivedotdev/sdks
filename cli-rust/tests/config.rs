...
impl ConfigDirEnvGuard {
    fn set(config_dir: Option<&str>, xdg_config_home: Option<&Path>, home: &Path) -> Self {
        let lock = ENV_LOCK.lock().expect("env lock");
        let previous_config_dir = std::env::var("PRIMITIVE_CONFIG_DIR").ok();
        ...
        match config_dir {
            Some(value) => std::env::set_var("PRIMITIVE_CONFIG_DIR", value),
            None => std::env::remove_var("PRIMITIVE_CONFIG_DIR"),
        }
...
fn write_credentials_lock_owner(config_dir: &Path, pid: u32) {
    fs::write(
        config_dir.join("credentials.lock").join("owner.json"),
        format!("{{\"pid\":{pid}}}\n"),
    )
...
#[cfg(unix)]
fn set_path_mtime(path: &Path, time: SystemTime) {
    use std::ffi::CString;
    use std::os::unix::ffi::OsStrExt;

    let duration = time.duration_since(UNIX_EPOCH).expect("mtime");
    let tv_sec = libc::time_t::try_from(duration.as_secs()).expect("mtime seconds");
    let tv_nsec = libc::c_long::from(duration.subsec_nanos());
    let times = [
        libc::timespec { tv_sec, tv_nsec },
        libc::timespec { tv_sec, tv_nsec },
    ];
    let path = CString::new(path.as_os_str().as_bytes()).expect("path contains nul");
...
#[test]
fn config_dir_matches_node_xdg_fallbacks() {
    let home = temp_config_dir("config-dir-home");
    let xdg = temp_config_dir("config-dir-xdg");

    {
        let _env = ConfigDirEnvGuard::set(None, Some(&xdg), &home);
        assert_eq!(config::config_dir(), xdg.join("primitive"));
    }

    {
        let _env = ConfigDirEnvGuard::set(Some(""), Some(&xdg), &home);
        assert_eq!(config::config_dir(), xdg.join("primitive"));
    }

    {
        let _env = ConfigDirEnvGuard::set(None, None, &home);
        assert_eq!(config::config_dir(), home.join(".config").join("primitive"));
    }
    ...
    fs::remove_dir_all(xdg).expect("remove xdg");
}
...
#[test]
fn resolve_auth_removes_legacy_api_key_credentials_and_continues_logged_out() {
    let config_dir = temp_config_dir("legacy-api-key-credentials");
    {
...
#[test]
fn resolve_auth_removes_blank_legacy_api_key_credentials_and_continues_logged_out() {
    let config_dir = temp_config_dir("blank-legacy-api-key-credentials");
    {
        let _env = ConfigEnvGuard::set(&config_dir);
        fs::write(
            config_dir.join("credentials.json"),
...
#[test]
fn resolve_auth_rejects_incomplete_saved_oauth_credentials_without_env_key() {
    let config_dir = temp_config_dir("incomplete-oauth-no-env-key");
    {
        let _env = ConfigEnvGuard::set(&config_dir);
        fs::write(
            config_dir.join("credentials.json"),
...
"#,
        ...
        let error = config::resolve_auth(&Default::default())
            .expect_err("incomplete saved OAuth should fail");

        assert!(
            error
                .to_string()
                .contains("refresh_token must be a non-empty string"),
            "{error:?}"
        );
    }
    fs::remove_dir_all(config_dir).expect("remove temp dir");
...
fn assert_refreshes_saved_oauth_credentials_with_expiry_value(
    name: &str,
    expires_at: Option<Value>,
) {
...
fn resolve_auth_refreshes_saved_oauth_credentials_with_missing_expiry() {
    assert_refreshes_saved_oauth_credentials_with_expiry_value(
        "refresh-missing-expiry-oauth",
        None,
    );
}

#[test]
fn resolve_auth_refreshes_saved_oauth_credentials_with_null_expiry() {
    assert_refreshes_saved_oauth_credentials_with_expiry_value(
        "refresh-null-expiry-oauth",
        Some(Value::Null),
    );
}
...
fn resolve_auth_refreshes_saved_oauth_credentials_with_blank_expiry() {
    assert_refreshes_saved_oauth_credentials_with_expiry_value(
        "refresh-blank-expiry-oauth",
        Some(json!("   ")),
    );
}

#[test]
fn resolve_auth_refreshes_saved_oauth_credentials_with_invalid_expiry() {
    assert_refreshes_saved_oauth_credentials_with_expiry_value(
        "refresh-invalid-expiry-oauth",
        Some(json!("not-a-timestamp")),
    );
}

#[test]
fn resolve_auth_rejects_saved_oauth_credentials_with_invalid_expiry_type() {
    let config_dir = temp_config_dir("reject-invalid-expiry-oauth");
    {
        let _env = ConfigEnvGuard::set(&config_dir);
        write_oauth_credentials_with_expiry(
            &config_dir,
        ...
        let error = config::resolve_auth(&Default::default())
            .expect_err("invalid expiry should be malformed");

        assert!(
            error
                .to_string()
                .contains("expires_at must be a string or null"),
            "{error:?}"
        );
    }
    fs::remove_dir_all(config_dir).expect("remove temp dir");
...
#[test]
fn config_set_accepts_attached_short_environment_value() {
    let config_dir = temp_config_dir("set-attached-short-env");
    {
        let _env = ConfigEnvGuard::set(&config_dir);
        ...
        .expect("set staging with attached short environment flag");
...
#[test]
fn config_reset_accepts_attached_short_environment_value() {
    let config_dir = temp_config_dir("reset-attached-short-env");
    {
        let _env = ConfigEnvGuard::set(&config_dir);
        write_config(
            &config_dir,
        ...
        friendly::dispatch(args(&["config", "reset", "-estaging"])).expect("reset staging");
...
#[cfg(unix)]
#[test]
fn config_use_recovers_stale_credentials_lock() {
    let config_dir = temp_config_dir("stale-lock");
    {
        let _env = ConfigEnvGuard::set(&config_dir);
        write_config(
            &config_dir,
        ...
        set_path_mtime(&lock_path, SystemTime::now() - Duration::from_secs(31 * 60));

        let result = config::config_use("staging").expect("recover stale lock");

        assert_eq!(result.environment, "staging");
        assert!(result.removed_credentials);
        let saved = read_config(&config_dir);
        assert_eq!(saved["current_environment"], "staging");
        assert!(!config_dir.join("credentials.json").exists());
        assert!(!config_dir.join("chat-state.json").exists());
...
#[cfg(unix)]
#[test]
fn config_use_recovers_dead_credentials_lock_owner() {
    let config_dir = temp_config_dir("dead-lock-owner");
    {
        let _env = ConfigEnvGuard::set(&config_dir);
        write_config(
            &config_dir,
        ...
        write_auth_files(&config_dir);
        fs::create_dir(config_dir.join("credentials.lock")).expect("create dead-owner lock");
        write_credentials_lock_owner(&config_dir, 999_999);

        let result = config::config_use("staging").expect("recover dead-owner lock");
...
#[cfg(unix)]
#[test]
fn config_use_keeps_live_credentials_lock_owner() {
    let config_dir = temp_config_dir("live-lock-owner");
    {
        let _env = ConfigEnvGuard::set(&config_dir);
        write_config(
            &config_dir,
        ...
        write_auth_files(&config_dir);
        let lock_path = config_dir.join("credentials.lock");
        fs::create_dir(&lock_path).expect("create live-owner lock");
        write_credentials_lock_owner(&config_dir, std::process::id());
        set_path_mtime(&lock_path, SystemTime::now() - Duration::from_secs(31 * 60));

        let error = config::config_use("staging").expect_err("live-owner lock should block");
