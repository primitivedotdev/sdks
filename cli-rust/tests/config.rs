...
#[test]
fn resolve_auth_rejects_saved_oauth_credentials_with_missing_expiry() {
    let config_dir = temp_config_dir("reject-missing-expiry-oauth");
    {
        let _env = ConfigEnvGuard::set(&config_dir);
        write_oauth_credentials_with_expiry(
            &config_dir,
        ...
        let error = config::resolve_auth(&Default::default())
            .expect_err("missing expiry should be malformed");

        assert!(
            error
                .to_string()
                .contains("expires_at must be a non-empty string"),
            "{error:?}"
        );
    }
    fs::remove_dir_all(config_dir).expect("remove temp dir");
...
#[test]
fn resolve_auth_rejects_saved_oauth_credentials_with_blank_expiry() {
    let config_dir = temp_config_dir("reject-blank-expiry-oauth");
    {
        let _env = ConfigEnvGuard::set(&config_dir);
        write_oauth_credentials_with_expiry(
            &config_dir,
        ...
        let error = config::resolve_auth(&Default::default())
            .expect_err("blank expiry should be malformed");
