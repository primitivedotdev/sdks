...
pub fn build_search_plan(command: &str, args: &[String]) -> Result<SearchCommandPlan> {
    match command {
        ...
        other => Err(crate::usage_err!("Unknown search command `{other}`")),
    }
}
...
pub fn execute_command(command: &str, args: &[String]) -> Result<()> {
    ...
    if has_json_flag(args) {
        ...
        print_search_table(plan.backend, &envelope);
    }
...
fn print_search_table(backend: SearchBackend, envelope: &Value) {
    let rows = envelope
        .get("data")
