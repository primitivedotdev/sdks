...
#[derive(Debug, Clone, PartialEq, Eq)]
enum DeployWaitOutcome {
    Deployed(Value),
    Failed(Value),
    Timeout { last_function: Option<Value> },
}
...
fn execute_deploy_api_plan(
    ...
    if wait {
        return match wait_for_deploy(
            ...
            DeployWaitOutcome::Deployed(function) => {
                print_json(&function)?;
                if create {
            ...
            DeployWaitOutcome::Failed(function) => {
                print_json(&function)?;
                let detail = function
                    .get("deploy_error")
                    .and_then(Value::as_str)
                ...
                eprintln!("Function {function_id} deploy failed{detail}");
                Ok(FunctionCommandExit::Code(1))
            }
            DeployWaitOutcome::Timeout { last_function } => {
                let status = last_function
                    .as_ref()
                    .and_then(|function| function.get("deploy_status"))
                    .and_then(Value::as_str)
                    .unwrap_or("unknown");
                eprintln!(
                    "Timed out after {timeout_seconds}s waiting for function {function_id} deploy to finish (last status: {status})."
                );
                Ok(FunctionCommandExit::Code(2))
            }
        };
    }
    print_json(&output)?;
...
fn wait_for_deploy(
    ...
    timeout_seconds: u64,
    poll_interval_seconds: u64,
) -> Result<DeployWaitOutcome> {
    let start = Instant::now();
    let mut last_function = initial;
    let mut last_status = last_function
        .as_ref()
        .and_then(deploy_status)
        .unwrap_or("unknown")
        .to_string();
    if let Some(function) = last_function.as_ref() {
        if let Some(outcome) = deploy_wait_terminal_outcome(function) {
            return Ok(outcome);
        }
    }
    eprintln!("Waiting for function {id} deploy to finish (current status: {last_status})...");

    loop {
        if has_wait_timeout_elapsed(start.elapsed(), timeout_seconds) {
            return Ok(DeployWaitOutcome::Timeout { last_function });
        }

        sleep_for_deploy_poll(start, timeout_seconds, poll_interval_seconds);
        if has_wait_timeout_elapsed(start.elapsed(), timeout_seconds) {
            return Ok(DeployWaitOutcome::Timeout { last_function });
        }

        let output = execute_function_request(&build_get_function_request(id), auth)?;
        if let Some(status) = deploy_status(&output) {
            if status != last_status {
                last_status = status.to_string();
                eprintln!("Function {id} deploy status: {status}");
            }
        }
        if let Some(outcome) = deploy_wait_terminal_outcome(&output) {
            return Ok(outcome);
        }
        last_function = Some(output);
    }
}

fn sleep_for_deploy_poll(start: Instant, timeout_seconds: u64, poll_interval_seconds: u64) {
...
fn deploy_wait_terminal_outcome(function: &Value) -> Option<DeployWaitOutcome> {
    match deploy_status(function) {
        Some("deployed") => Some(DeployWaitOutcome::Deployed(function.clone())),
        Some("failed") => Some(DeployWaitOutcome::Failed(function.clone())),
        _ => None,
    }
}

fn deploy_status(function: &Value) -> Option<&str> {
    function.get("deploy_status").and_then(Value::as_str)
}
...
fn print_command_help(command: &str) -> bool {
    match function_command_kind(command) {
        ...
        Some(FunctionCommandKind::Test) => {
            let command = if matches!(command, "test-function" | "functions:test-function") {
                "test-function"
            } else {
                "test"
            };
            print!("{}", functions_test_help_text_for(command));
            true
        }
...
pub fn functions_test_help_text() -> String {
    functions_test_help_text_for("test")
}

fn functions_test_help_text_for(command: &str) -> String {
    let bin = crate::display_bin_name();
    format!(
        r#"Trigger a test invocation; with --wait, watch it land

USAGE
  {bin} functions {command} --id <fn-id> [--api-key <value>] [--local-part <value>] [--wait] [--show-sends] [--timeout <seconds>] [--poll-interval <seconds>] [--time]
...
pub fn parse_logs_command_plan(args: &[String]) -> Result<FunctionLogsPlan> {
    ...
    if limit == 0 {
        return Err(crate::usage_error("--limit must be greater than 0."));
    }
...
fn parse_args(
    ...
    while index < args.len() {
        ...
        if repeatable_value_flags.contains(name) {
            ...
            return Err(crate::usage_error(format!("Pass --{name} only once.")));
        }
        index += 1;
    }
