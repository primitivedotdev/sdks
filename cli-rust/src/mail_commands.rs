...
const CHAT_LOCK_FILE: &str = "chat-state.lock";
...
fn build_chat_command_plan_from_args_internal(
    ...
    let derive_from_from_parent = reply_mode
        && reply_to_email_id.is_some()
        && explicit_from.is_none()
        && default_from.is_none();
    let allow_default_from_lookup = !reply_mode || reply_to_email_id.is_none();
    let from = if derive_from_from_parent {
        String::new()
    } else {
        resolve_outbound_from(
            explicit_from,
            default_from,
    ...
    let send = if (reply_mode && reply_to_email_id.is_none()) || derive_from_from_parent {
        None
    } else {
    ...
    SleepFn: FnMut(u64) -> Result<()>,
{
    ...
    let reply_match = if sent.get("idempotent_replay").and_then(Value::as_bool) == Some(true) {
        ...
        .ok_or_else(|| {
            let recovery_context = format_chat_recovery_context(&ChatRecoveryContext {
                from: &reply_address,
...
fn format_chat_recovery_context(context: &ChatRecoveryContext<'_>) -> String {
    let accepted =
        accepted_recipients(context.sent).unwrap_or_else(|| context.recipient.to_string());
    let delivery_status = string_field(context.sent, "delivery_status")
        .or_else(|| string_field(context.sent, "status"))
        .unwrap_or_else(|| "unknown".to_string());
    let mut lines = vec![
        String::new(),
        "Sent message context".to_string(),
        format!("  To: {accepted}"),
        format!(
            "  From: {}",
            string_field(context.sent, "from").unwrap_or_else(|| context.from.to_string())
        ),
        format!("  Subject: {}", context.subject),
        format!(
            "  Sent email id: {}",
            string_field(context.sent, "id").unwrap_or_else(|| "(unknown)".to_string())
        ),
        format!("  Delivery status: {delivery_status}"),
        format!("  Poll since: {}", context.sent_at_iso),
        String::new(),
        "Helpful recovery commands".to_string(),
    ];
    for command in build_chat_recovery_commands(context) {
...
pub fn format_chat_transcript(outcome: &ChatCommandOutcome) -> String {
    ...
    lines.extend([
        ...
        "  Commands are templates; use --json for parse-safe output.".to_string(),
        "  When shown, --strict-only prefers timing out over matching the wrong reply.".to_string(),
    ]);
    ...
    lines.extend([
        ...
        format!(
            ...
            chat_response_body_format_label(outcome.response_body_format)
        ),
...
fn parse_args(
    ...
    while index < args.len() {
        ...
        if bool_flags.contains(name) {
            ...
            parsed.bool_flags.insert(name.to_string(), true);
            index += 1;
            continue;
        }
        if !value_flags.contains(name) && !repeatable_value_flags.contains(name) {
            return Err(crate::usage_error(format!("Nonexistent flag: --{name}")));
        }
        ...
        let value = if let Some(value) = inline_value {
            ...
            index += 1;
            next_value_arg(args, index, name)?.clone()
        };
        if repeatable_value_flags.contains(name) {
            ...
            return Err(crate::usage_error(format!(
                "Flag --{name} can only be specified once"
            )));
        }
        index += 1;
    }
...
fn next_value_arg<'a>(args: &'a [String], index: usize, name: &str) -> Result<&'a String> {
    let value = args
        .get(index)
        .ok_or_else(|| crate::usage_error(format!("Flag --{name} expects a value")))?;
    if value.starts_with("--") {
...
fn execute_chat_command_plan(plan: &ChatCommandPlan) -> Result<()> {
    ...
    let _chat_lock = acquire_chat_lock(&config_dir)?;
    let auth = config::resolve_auth(&plan.runtime.auth)?;
...
static CHAT_LOCK_HOLDER: OnceLock<Mutex<Option<ChatLockHolder>>> = OnceLock::new();
...
impl Drop for ChatLockGuard {
    fn drop(&mut self) {
        ...
        let holder_lock = chat_lock_holder();
        let Ok(mut holder) = holder_lock.lock() else {
            return;
        };
        let Some(current) = holder.as_mut() else {
            return;
        };
        if current.config_dir != self.config_dir {
            return;
        }
        ...
        let _ = fs::remove_file(chat_lock_path(&self.config_dir));
        *holder = None;
        self.active = false;
    }
}

fn chat_lock_holder() -> &'static Mutex<Option<ChatLockHolder>> {
    CHAT_LOCK_HOLDER.get_or_init(|| Mutex::new(None))
}

fn acquire_chat_lock(config_dir: &Path) -> Result<ChatLockGuard> {
    let config_dir = config_dir.to_path_buf();
    let holder_lock = chat_lock_holder();
    let mut holder = holder_lock
        .lock()
        .map_err(|_| anyhow!("Could not acquire Primitive chat lock."))?;
    if let Some(current) = holder.as_mut() {
        if current.config_dir == config_dir {
            current.depth += 1;
            return Ok(ChatLockGuard {
                config_dir,
                active: true,
            });
        }
    ...
    config::ensure_private_config_dir(&config_dir)?;
    let path = chat_lock_path(&config_dir);
    for _ in 0..2 {
        match create_chat_lock_file(&path) {
            Ok(()) => {
                *holder = Some(ChatLockHolder {
            ...
            Err(error) if error.kind() == std::io::ErrorKind::AlreadyExists => {
                let holder_pid = read_chat_lock_pid(&path);
                if holder_pid.is_some_and(pid_is_alive) {
                    let pid = holder_pid.expect("checked is_some");
                    return Err(anyhow!(
                        "Another `primitive chat` invocation (pid {pid}) is in progress. Wait for it to finish, or kill it before retrying."
                    ));
                }
    ...
    Err(anyhow!(
        "Could not acquire Primitive chat lock at {}.",
        path.display()
    ))
}

fn chat_lock_path(config_dir: &Path) -> PathBuf {
    config_dir.join(CHAT_LOCK_FILE)
}

fn create_chat_lock_file(path: &Path) -> std::io::Result<()> {
...
fn read_chat_lock_pid(path: &Path) -> Option<u32> {
    ...
    RequestFn: FnMut(&ApiRequest) -> Result<Value>,
{
    match &plan.reply_context {
        ...
        ChatReplyContextPlan::Exact { id, request } => {
            ...
            let from = required_string_field(&parent, "to_email", "Reply context email")?;
            build_chat_send_request(&ChatSendInput {
                ...
                reply_to_email_id: Some(id.clone()),
            })
        }
...
fn chat_response_body_format_label(format: ChatResponseBodyFormat) -> &'static str {
    match format {
        ChatResponseBodyFormat::Empty => "empty",
        ChatResponseBodyFormat::Html => "html",
        ChatResponseBodyFormat::Text => "text",
    }
}
...
fn build_chat_recovery_commands(context: &ChatRecoveryContext<'_>) -> Vec<ChatFollowUpCommand> {
    let sent_id = string_field(context.sent, "id").unwrap_or_else(|| "(unknown)".to_string());
    let mut commands = vec![chat_follow_up_command(
        "wait_threaded_reply",
        "Wait for the threaded reply again",
        vec![
            "primitive".to_string(),
            "emails".to_string(),
            "wait".to_string(),
            ...
            sent_id.clone(),
            "--to".to_string(),
            context.from.to_string(),
            "--since".to_string(),
            context.sent_at_iso.to_string(),
            "--timeout".to_string(),
            context.timeout_seconds.to_string(),
        ],
        false,
    )];
    if !context.strict_only {
        commands.push(chat_follow_up_command(
            "wait_fallback_reply",
    ...
    commands.push(chat_follow_up_command(
        "inspect_sent_email",
        "Inspect the outbound send",
        string_args(&["primitive", "sent", "get", "--id", &sent_id]),
        false,
    ));
    commands
}
