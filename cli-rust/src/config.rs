...
pub const SAVED_CLI_OAUTH_SESSION_EXPIRED_MESSAGE: &str =
    "Saved Primitive CLI OAuth session expired or was revoked. Run `primitive signin` to authenticate again.";
pub const SAVED_CLI_OAUTH_SESSION_REJECTED_MESSAGE: &str = "Your saved Primitive CLI OAuth session was rejected. If the command was working a moment ago, please retry; brief retries often clear transient rejections. If it keeps failing, run `primitive logout && primitive signin` to mint a fresh session.";
...
fn resolve_auth_with_options(
    ...
    let (api_key, source) = if let Some(api_key) = flag_or_env_api_key {
        (Some(api_key), AuthSource::Explicit)
    } else if let Some(access_token) = credentials
        .as_ref()
        .and_then(|item| item.access_token.clone())
    {
        (Some(access_token), AuthSource::Stored)
    } else {
...
fn validate_stored_credentials(raw: &Value) -> Result<()> {
    ...
    if !matches!(
        ...
        return Err(malformed_credentials("expires_at must be a string or null"));
    }
