...
pub fn build_charge_request(args: &[String]) -> Result<ChargeRequest> {
    let invocation = api::parse_invocation(args)?;
    ...
    let amount = if let Some(value) = invocation.flags.get("amount-usdc") {
        let base_units = crate::x402::usdc_to_base_units(value)?;
...
fn execute_charge(args: &[String]) -> Result<()> {
    let request = build_charge_request(args)?;
    let invocation = api::parse_invocation(args)?;
...
fn execute_pay_email(args: &[String]) -> Result<()> {
    ...
    if replayed {
        eprintln!("Primitive returned an idempotent replay for this payment send; no fresh payment email was sent.");
    }
...
fn print_help() {
    ...
    println!("  {bin} payments spend-policy get|update");
