...
fn build_charge_request_from_invocation(invocation: &api::Invocation) -> Result<ChargeRequest> {
    ...
    validate_charge_network(&network)?;
...
fn validate_charge_network(network: &str) -> Result<()> {
    match network {
        "base-sepolia" | "base" => Ok(()),
        _ => Err(crate::usage_error(format!(
            "Expected --network={network} to be one of: base, base-sepolia"
        ))),
    }
}
...
fn request_json_raw(
    ...
    if json.is_none() && !bytes.is_empty() {
        return Err(payment_non_json_response_error(status, path, &bytes));
    }
    if status >= 400 {
...
fn payment_non_json_response_error(status: u16, path: &str, bytes: &[u8]) -> anyhow::Error {
    let body = String::from_utf8_lossy(bytes);
    let display_path = if path == "/v1" || path.starts_with("/v1/") {
        path.to_string()
    } else {
        format!("/v1{path}")
    };
    anyhow!(
        ...
        serde_json::to_string_pretty(&json!({
            ...
            "error": {
                "code": "request_failed",
                "message": format!("non-JSON response ({status}) from {display_path}: {body}"),
            },
        }))
        .expect("payment error JSON should serialize")
    )
}
