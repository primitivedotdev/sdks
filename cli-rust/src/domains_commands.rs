...
pub fn select_domain_by_name(domains_response: &Value, domain: &str) -> Result<DomainSelection> {
    ...
    for row in rows {
        ...
        } else if row_domain.eq_ignore_ascii_case(domain) {
            folded_matches.push(row);
        }
    }
...
fn execute_zone_file_command_plan(
    ...
    write_zone_file_output(&plan.output, &bytes)?;
    Ok(())
}
...
fn parse_args(
    ...
    while index < args.len() {
        ...
        let value = if let Some(value) = inline_value {
            ...
            let value = args
                .get(index)
                .ok_or_else(|| crate::usage_err!("Missing value for -{short_name}"))?;
            if value.starts_with("--") {
                return Err(crate::usage_err!("Missing value for -{short_name}"));
            }
            value.clone()
        };
        insert_parsed_value(&mut parsed, name, value)?;
