...
pub fn build_domains_command_plan(
    ...
    let kind = domains_command_kind(command)
        .ok_or_else(|| crate::usage_error(format!("Unknown domains command `{command}`")))?;
    match kind {
...
fn execute_json_request(request: &DomainsApiRequest, auth: &config::ResolvedAuth) -> Result<Value> {
    let (_, json) = execute_request(request, auth, true)?;
    json.ok_or_else(|| anyhow!("HTTP 200 returned no JSON body"))
}
...
fn execute_binary_request(
    ...
    let (bytes, _) = execute_request(request, auth, false)?;
    Ok(bytes)
}
...
fn execute_request(
    ...
    let (status, bytes, json) = if error_on_declared_json_parse_failure {
        client::parse_response_with_declared_json_error(response)?
    } else {
        client::parse_binary_response(response)?
    };
    if status >= 400 {
