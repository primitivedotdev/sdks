    ...
    F: Fn() -> RequestBuilder,
{
    for attempt in 0..=DEFAULT_MAX_RETRIES {
        ...
        let backoff_ms = (1000u64.saturating_mul(2u64.saturating_pow(attempt as u32))).min(15_000);
...
fn api_root(base_url: &str) -> String {
    ...
    if trimmed.ends_with("/v1") {
        format!("{trimmed}/payloads")
    } else {
        format!("{trimmed}/v1/payloads")
    }
}
...
fn resolve_client(flags: &BTreeMap<String, String>) -> Result<PayloadClientOptions> {
    let auth = crate::config::resolve_auth(flags)?;
    ...
    match command.and_then(payload_command_kind) {
        Some(PayloadCommandKind::Push) => format!(
            "Stream-upload a file as an encrypted payload\n\
             ...
             FLAGS\n\
               ...
               --concurrency=<value>  Parallel chunk uploads.\n\
        ...
        Some(PayloadCommandKind::Pull) => format!(
            "Stream-download and decrypt a payload to a file\n\
             \n\
             USAGE\n\
               {bin} payloads pull <merkle_root> --out <value> --cek <value> [--api-key <value>] [--quiet]\n\
             \n\
             ARGUMENTS\n\
               <merkle_root>  Object content address.\n\
             \n\
             FLAGS\n\
               --api-key=<value>  Primitive API key override.\n\
               --cek=<value>      Hex content-encryption key from payloads push.\n\
               --out=<value>      Output file path.\n\
               --quiet            Suppress progress output.\n"
        ),
        None => format!(
            "Primitive payload commands\n\
             \n\
             USAGE\n\
