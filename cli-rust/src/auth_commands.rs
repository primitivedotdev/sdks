...
pub fn dispatch(args: &[String]) -> Result<AuthCommand> {
    let (command, rest) = args
        .split_first()
        .ok_or_else(|| crate::usage_error("Missing auth command."))?;
    match command.as_str() {
        ...
        other => Err(crate::usage_error(format!("Unknown auth command: {other}"))),
    }
}
    ...
    match args.first().map(String::as_str) {
        ...
        _ => {
            ...
            if parsed.positionals.is_empty() {
                let start_flags = start_flags_without_env_signup_code(&parsed);
                ...
                if parsed.bool("no-browser") {
                    return Err(crate::usage_error("Unknown flag --no-browser"));
                }
...
fn start_flags(parsed: &ParsedArgs) -> StartEmailCodeFlags {
    StartEmailCodeFlags {
        ...
        force: parsed.bool("force"),
        signup_code: non_empty_optional(parsed.value("signup-code")).or_else(env_signup_code),
    }
}

fn start_flags_without_env_signup_code(parsed: &ParsedArgs) -> StartEmailCodeFlags {
    StartEmailCodeFlags {
        accept_terms: parsed.bool("accept-terms"),
        api_base_url: parsed.value("api-base-url"),
        device_name: parsed.value("device-name"),
...
fn env_signup_code() -> Option<String> {
    std::env::var("PRIMITIVE_SIGNUP_CODE")
        .ok()
        .and_then(|value| non_empty_optional(Some(value)))
}
        ...
        while index < args.len() {
            ...
            if arg == "-f" {
                if !bool_flags.contains(&"force") {
                    return Err(crate::usage_error("Unknown flag -f"));
                }
            ...
            if let Some(name) = arg.strip_prefix("--no-") {
                ...
                return Err(crate::usage_error(format!("Unknown flag --no-{name}")));
            }
...
fn validate_verification_code_sources(
    ...
    if sources.is_empty() {
        return Err(crate::usage_error("Pass the verification code as a positional argument or via one of --code-from-stdin, --code-from-file, or --code-from-env."));
    }
    if sources.len() > 1 {
        return Err(crate::usage_error(format!(
...
pub fn required_signup_code(
    ...
    match (
        ...
        (false, None) => Ok(None),
        (true, None) => Err(crate::usage_error(format!(
    ...
    match command {
        ...
        AuthCommand::StartEmailCode { email, flags, flow } => {
            let email = email.as_deref().ok_or_else(|| {
                crate::usage_error(format!("{} needs an email address.", flow.action_noun()))
            })?;
...
pub trait AuthRuntimeIo {
    ...
    fn read_stdin_to_string(&mut self) -> Result<String> {
        ...
        if stdin.is_terminal() {
            return Err(crate::usage_error("stdin is a TTY; pipe the code into this command or use --code-from-file / --code-from-env instead."));
        }
        let mut value = String::new();
        stdin
...
pub fn execute_auth_command(command: &AuthCommand) -> Result<()> {
    ...
    let context = auth_execution_context_for_command(command)?;
...
fn auth_execution_context_for_command(command: &AuthCommand) -> Result<AuthExecutionContext> {
    let fallback_device_name = fallback_device_name();
    let now = SystemTime::now();
    let flags = auth_flags_from_command(command);
    if matches!(
        command,
        AuthCommand::Logout { .. } | AuthCommand::Whoami { .. }
    ) {
        let auth = config::resolve_auth(&flags)?;
        return Ok(AuthExecutionContext::from_resolved_auth(
            &auth,
    ...
    let env_api_base_url = env::var("PRIMITIVE_API_BASE_URL")
        .ok()
    ...
    let request_config = config::resolve_request_config(&config_dir, api_base_url)?;
    Ok(AuthExecutionContext {
        api_base_url: request_config.api_base_url,
        api_key: None,
...
pub fn execute_auth_request(
    ...
    let auth = config::ResolvedAuth {
        ...
        source: if context.api_key.is_some() {
            config::AuthSource::Explicit
        } else {
    ...
    let headers = response
        ...
        .collect();
    let (status, _bytes, body) = client::parse_response_with_declared_json_error(response)?;
    Ok(AuthApiResponse {
        body,
        headers,
        status,
...
fn probe_existing_login(
    ...
    io: &mut impl AuthRuntimeIo,
    http: &mut impl AuthRuntimeHttp,
) -> Result<ExistingLoginProbeStatus> {
    let probe_context = AuthExecutionContext {
    ...
    let response = http.send(&probe_context, &plan_whoami_account_request())?;
    if response_has_data(&response) {
        return Ok(ExistingLoginProbeStatus::Valid);
    }

    if response_error_code(&response) == Some("unauthorized")
        && normalize_auth_base_url(&context.api_base_url)
            == normalize_auth_base_url(&credentials.api_base_url)
    {
        delete_cli_credentials(io, &context.config_dir)?;
        return Ok(ExistingLoginProbeStatus::RemovedStale);
    }

    let message = if response_error_code(&response) == Some("unauthorized") {
    ...
    Ok(ExistingLoginProbeStatus::Blocked {
        message: message.to_string(),
    })
}

fn normalize_auth_base_url(value: &str) -> String {
    config::normalize_api_base_url(Some(value))
}
...
fn read_credentials_for_auth(
    ...
    if is_legacy_api_key_credentials_value(&value) {
        io.remove_file(&path)?;
        io.remove_file(&chat_state_path(config_dir))?;
        return Ok(None);
    }
    let credentials = serde_json::from_value(value)
        .with_context(|| format!("Stored Primitive CLI credentials are malformed: {path}"))?;
    Ok(Some(credentials))
}

fn is_legacy_api_key_credentials_value(value: &Value) -> bool {
    ...
    ["api_key", "key_id", "base_url"].iter().any(|field| {
        object
            .get(*field)
            .and_then(Value::as_str)
...
fn auth_error_for_response(response: &AuthApiResponse, fallback: &str) -> anyhow::Error {
    if let Some(body) = &response.body {
        ...
        return client::error_for_status_with_hints(response.status, Some(body), &bytes);
    }
    if response.status >= 400 {
        return client::error_for_status_with_hints(response.status, None, &[]);
    }
    anyhow!("{fallback}")
}
