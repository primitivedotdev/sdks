...
fn is_generated_cli_alias(id: &str) -> bool {
    ...
    generated_cli_aliases()
        .iter()
        .any(|(alias, _)| *alias == id || normalize_lookup_token(alias) == normalized)
}
...
pub fn uses_generated_cli_help(id: &str) -> bool {
    let resolved = resolve_alias(id);
    if is_overridden_operation_id(resolved) {
        return false;
    }
    if lookup_operation(id).is_none() {
        return false;
    }
    resolved == id || is_generated_cli_alias(id)
}
