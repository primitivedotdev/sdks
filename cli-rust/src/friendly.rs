...
pub fn dispatch(args: Vec<String>) -> Result<()> {
    ...
    if let Some(snapshot) = crate::help_snapshots::lookup(&args) {
        print!("{snapshot}");
        return Ok(());
    }
    if args[0] == "--help" {
        if args.len() == 1 || args[1..].iter().all(|arg| arg.starts_with('-')) {
            print_help();
            return Ok(());
        }
        let mut routed = args[1..].to_vec();
        if !is_help_request(&routed) {
            routed.push("--help".to_string());
        }
        return dispatch(routed);
    }
    if args[0] == "help" && args.len() > 1 {
        if args[1..].iter().any(|arg| arg == "-h") {
            return Err(crate::usage_error("Nonexistent flag: -h"));
        }
        let mut routed = args[1..].to_vec();
        routed.push("--help".to_string());
        return dispatch(routed);
    ...
    if is_short_help_only(rest) && !is_topic_help_snapshot(&command) && command != "list-operations"
    {
        return Err(crate::usage_error("Nonexistent flag: -h"));
    }
    if is_help_request(rest) && !(command == "list-operations" && is_short_help_only(rest)) {
        if print_normalized_help_snapshot(&command, rest) {
            return Ok(());
        }
        ...
        if manifest::uses_generated_cli_help(&command) {
            let operation =
                manifest::lookup_cli_operation(&command).expect("generated help operation");
            print!(
                "{}",
    ...
    match command.as_str() {
        "list-operations"
            if rest
                .first()
                .is_some_and(|arg| matches!(arg.as_str(), "--help")) =>
        {
        ...
        other => {
            ...
            if let Some(operation) = manifest::lookup_cli_operation(other) {
                ...
                Err(crate::usage_error(format!("Command {other} not found")))
            }
        }
    }
}

fn print_normalized_help_snapshot(command: &str, rest: &[String]) -> bool {
    let help_index = rest
        .iter()
        .position(|arg| matches!(arg.as_str(), "--help" | "-h"))
        .unwrap_or(rest.len());
    let has_positionals_before_help = rest[..help_index].iter().any(|arg| !arg.starts_with('-'));
    if has_positionals_before_help && !allows_positionals_before_help(command) {
        return false;
    }
...
fn split_command(args: &[String]) -> Result<(String, &[String])> {
    if is_help_request(args) {
        let flag_index = args
            ...
            .unwrap_or(args.len());
        if flag_index > 1 {
            let candidate = args[..flag_index].join(":");
            if is_known_command_candidate(&candidate) {
                return Ok((candidate, &args[flag_index..]));
            }
            if let Some((prefix, prefix_len)) = longest_known_prefix(&args[..flag_index]) {
                if allows_positionals_before_help(&prefix)
                    || (prefix_len == 1
                        && flag_index == 2
                        && routes_unknown_help_to_parent(&prefix))
                {
                    return Ok((prefix, &args[prefix_len..]));
                }
            }
            return Ok((candidate, &args[flag_index..]));
        }
        if flag_index == 1 && is_known_command_candidate(&args[0]) {
            return Ok((args[0].clone(), &args[1..]));
        }
    }
...
fn longest_known_prefix(args: &[String]) -> Option<(String, usize)> {
    for length in (1..=args.len()).rev() {
        let candidate = args[..length].join(":");
        if is_known_command_candidate(&candidate) {
...
fn allows_positionals_before_help(command: &str) -> bool {
    matches!(
        command,
        "autocomplete"
            | "chat"
...
fn routes_unknown_help_to_parent(command: &str) -> bool {
    matches!(command, "emails" | "payments")
}
...
fn is_short_help_only(args: &[String]) -> bool {
    args.iter().any(|arg| arg == "-h") && !args.iter().any(|arg| arg == "--help")
}

fn is_topic_help_snapshot(command: &str) -> bool {
    let mut args = command
        .split(':')
        .map(str::to_string)
        .collect::<Vec<String>>();
    args.push("--help".to_string());
    let Some(snapshot) = crate::help_snapshots::lookup(&args) else {
        return false;
    };
    let usage = format!("$ primitive {} COMMAND", command.replace(':', " "));
    snapshot.contains(&usage) && snapshot.contains("\nCOMMANDS\n")
}

fn is_known_command_candidate(candidate: &str) -> bool {
    manifest::lookup_cli_operation(candidate).is_some()
        ...
        || crate::search_commands::is_search_command(candidate)
...
fn print_help() {
    let root_help_args = [String::from("--help")];
    if let Some(snapshot) = crate::help_snapshots::lookup(&root_help_args) {
        print!("{snapshot}");
        return;
    }
