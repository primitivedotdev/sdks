...
`trusted` requires a `legit` verdict and a strict-parsed single From address in the exact expected `domain` (and exact `sender` when given). `reason` names the first check that failed.
...
Use `deleteSentEmail` and `removeAgentConnection` from `@primitivedotdev/sdk/api`.
