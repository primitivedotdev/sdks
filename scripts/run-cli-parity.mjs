...
function normalizeMaskedPathSeparators(value) {
  return value.replace(MASKED_PATH_PATTERN, (match) => match.replace(/[\\/]+/g, "/"));
}
...
function renderComparableStream(value, normalizers) {
  ...
  if (parsed.ok) {
    return asJsonValue(deepSort(normalizeComparableStrings(parsed.value, normalizers)));
  }
  return normalizeMaskedPathSeparators(masked);
}

function renderComparableFragment(value, normalizers) {
  return normalizeMaskedPathSeparators(applyMasks(normalizeScalarString(value), normalizers));
}
...
function responseBody(response) {
  if (response.bodyBase64 !== undefined) return Buffer.from(response.bodyBase64, "base64");
...
async function runProcess(command, args, env, cwd, stdin, timeoutMs, verbose) {
  ...
  const executable = words.length === 1 && words[0].endsWith(".js") ? process.execPath : words[0];
  const childArgs =
  ...
  if (result.error) {
    if (result.error.name === "AbortError") {
      return { exitCode: 124, stderr: `${stderr}\nTimed out after ${timeoutMs}ms\n`, stdout };
    }
    return { exitCode: 127, stderr: `${stderr}${result.error.message}\n`, stdout };
  }
  return {
    exitCode: result.signalName ? 128 : (result.code ?? 0),
    stderr: stdinError ? `${stderr}${stdinError.message}\n` : stderr,
    stdout,
  };
}
...
function baseEnv(context, caseEnv) {
  ...
  return { ...env, ...expand(caseEnv ?? {}, context) };
}
...
function writeFixtureFiles(files, context) {
  for (const [relativePath, value] of Object.entries(files ?? {})) {
    ...
    if (!target.startsWith(context.caseTmp)) {
      throw new Error(`Refusing to write fixture outside case tmp: ${relativePath}`);
    }
...
function fileContentsFromSpec(value, context) {
  ...
  return typeof value === "string" ? expand(value, context) : asJsonValue(expand(value, context));
}
...
function writeConfigFiles(files, context) {
  for (const [relativePath, value] of Object.entries(files ?? {})) {
    ...
    if (!target.startsWith(context.configDir)) {
      throw new Error(`Refusing to write config outside config dir: ${relativePath}`);
    }
    mkdirSync(path.dirname(target), { recursive: true });
    const contents =
      typeof value === "string" ? expand(value, context) : asJsonValue(expand(value, context));
    writeFileSync(target, contents);
  }
}
...
function normalizersFor(context, caseNormalizers) {
  ...
  for (const mask of caseNormalizers?.masks ?? []) {
    if (typeof mask === "string") {
      regexMasks.push({ regex: new RegExp(mask, "g"), replacement: "<masked>" });
    } else {
      regexMasks.push({
...
function expectedTextFrom(spec, context) {
  ...
  if (spec.file) {
    return expand(readFileSync(path.resolve(REPO_ROOT, spec.file), "utf8"), context);
  }
  if (spec.json !== undefined) return asJsonValue(deepSort(expand(spec.json, context)));
  if (
...
function assertStreamExpectation(label, actual, spec, context, normalizers) {
  ...
  if (spec.contains) {
    for (const item of spec.contains) {
      ...
      const comparableExpected = renderComparableFragment(expected, normalizers);
      assert(
        actual.includes(expected) || comparableActual.includes(comparableExpected),
  ...
  if (spec.matches) {
    for (const item of spec.matches) {
      ...
      assert(new RegExp(pattern, flags).test(actual), `${label} did not match /${pattern}/${flags}.\nActual:\n${actual}`);
    }
  }
...
const JSON_MATCHER_KEYS = new Set(["$flags", "$matches", "$notEqual", "$notMatches"]);

function isJsonMatcher(value) {
  if (!value || typeof value !== "object" || Array.isArray(value)) return false;
  const keys = Object.keys(value);
  return keys.some((key) => key === "$matches" || key === "$notEqual" || key === "$notMatches")
    && keys.every((key) => JSON_MATCHER_KEYS.has(key));
}

function regexFromJsonMatcher(matcher, key, label) {
  assert.equal(typeof matcher[key], "string", `${label} ${key} must be a string`);
  if (matcher.$flags !== undefined) {
    assert.equal(typeof matcher.$flags, "string", `${label} $flags must be a string`);
  }
...
function assertJsonMatches(actual, expected, label) {
  if (isJsonMatcher(expected)) {
    if (expected.$matches !== undefined) {
      ...
      assert.equal(typeof actual, "string", `${label} expected a string matching /${regex.source}/${regex.flags}`);
      regex.lastIndex = 0;
      assert(regex.test(actual), `${label} did not match /${regex.source}/${regex.flags}. Actual: ${JSON.stringify(actual)}`);
    }
    if (expected.$notMatches !== undefined) {
      const regex = regexFromJsonMatcher(expected, "$notMatches", label);
      assert.equal(typeof actual, "string", `${label} expected a string not matching /${regex.source}/${regex.flags}`);
      regex.lastIndex = 0;
      assert(!regex.test(actual), `${label} unexpectedly matched /${regex.source}/${regex.flags}. Actual: ${JSON.stringify(actual)}`);
    }
    if (expected.$notEqual !== undefined) {
      assert.notDeepEqual(actual, expected.$notEqual, `${label} unexpectedly equaled ${JSON.stringify(expected.$notEqual)}`);
    }
    return;
  }
  ...
  if (expected && typeof expected === "object") {
    assert(actual && typeof actual === "object" && !Array.isArray(actual), `${label} expected an object`);
    assert.deepEqual(Object.keys(actual).sort(), Object.keys(expected).sort(), `${label} object keys mismatch`);
...
function assertRequestMatches(actual, expected, context) {
  ...
  if (expanded.method !== undefined) assert.equal(actual.method, expanded.method);
  if (expanded.path !== undefined) assertJsonMatches(actual.path, expanded.path, "path");
  if (expanded.query !== undefined) {
    assertJsonMatches(deepSort(actual.query), deepSort(expanded.query), "query");
  }
  if (expanded.body !== undefined) assertJsonMatches(actual.body, expanded.body, "body");
  ...
  if (expanded.headers !== undefined) {
    assert.deepEqual(pickHeaders(actual.headers, expanded.headers), lowerHeaderObject(expanded.headers));
  }
...
function comparableRequest(actual, expected, context) {
  ...
  if (expanded.path !== undefined) result.path = comparableJson(actual.path, expanded.path);
  if (expanded.query !== undefined) {
    result.query = deepSort(comparableJson(actual.query, expanded.query));
  }
  if (expanded.body !== undefined) result.body = deepSort(comparableJson(actual.body, expanded.body));
  if (expanded.bodyBase64 !== undefined) {
  ...
  for (const name of expanded.headersAbsent ?? []) {
    result[`header:${String(name).toLowerCase()}`] = actual.headers[String(name).toLowerCase()] ?? null;
  }
  return deepSort(result);
}
...
function comparableRequests(actual, expected, context) {
  ...
  return actual.map((request, index) => comparableRequest(request, expected[index], context));
}
...
function assertOutputFiles(files, context, normalizers) {
  for (const [pathSpec, spec] of Object.entries(files ?? {})) {
    ...
    if (isBase64FileSpec(spec)) {
      ...
      assertJsonMatches(actual, expand(spec.base64, context), `output file ${pathSpec} base64`);
    } else {
      const actual = readFileSync(target, "utf8");
      assertStreamExpectation(`output file ${pathSpec}`, actual, spec, context, normalizers);
    }
  }
}
...
function assertConfigFiles(files, context) {
  for (const [relativePath, expected] of Object.entries(files ?? {})) {
    const target = path.resolve(context.configDir, relativePath);
    if (!target.startsWith(context.configDir)) {
      throw new Error(`Refusing to read config outside config dir: ${relativePath}`);
    }
    const raw = readFileSync(target, "utf8");
    if (typeof expected === "string") {
      assert.equal(raw, expand(expected, context), `config file ${relativePath} mismatch`);
    } else {
      assertJsonMatches(
...
function assertConfigFilesAbsent(files, context) {
  for (const relativePath of files ?? []) {
    const target = path.resolve(context.configDir, relativePath);
    if (!target.startsWith(context.configDir)) {
      throw new Error(`Refusing to inspect config outside config dir: ${relativePath}`);
    }
...
async function runSide({ binary, caseItem, commandName, options, tmpRoot }) {
  ...
  try {
    ...
    assert.equal(result.exitCode, expected.exitCode ?? 0, `${commandName} exit code mismatch\nstderr:\n${result.stderr}`);
    assertStreamExpectation(`${commandName} stdout`, result.stdout, expected.stdout, context, normalizers);
    assertStreamExpectation(`${commandName} stderr`, result.stderr, expected.stderr, context, normalizers);
    assertRequests(server.requests, expectedRequests, context);
...
async function runCase(caseItem, options, tmpRoot) {
  ...
  if (caseItem.compare !== false) {
    assert.deepEqual(rust.comparable, node.comparable, "Node/Rust process output parity mismatch");
    assert.deepEqual(rust.requests, node.requests, "Node/Rust HTTP request parity mismatch");
  }
...
async function main() {
  ...
  if (!Array.isArray(cases)) throw new Error("Fixture must be an array or { cases: [...] }.");
  ...
  try {
    ...
    else rmSync(tmpRoot, { recursive: true, force: true, maxRetries: 5, retryDelay: 100 });
  }
}
