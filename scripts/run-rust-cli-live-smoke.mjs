...
function parseArgs(argv) {
  const options = {
    ...
    apiBaseUrl:
      process.env.PRIMITIVE_API_BASE_URL ?? "https://api.primitive.dev/v1",
    ...
    includeSecretArgv:
      process.env.PRIMITIVE_RUST_LIVE_INCLUDE_SECRET_ARGV === "1",
    keepTmp: false,
    noKeyOnly: process.env.PRIMITIVE_RUST_LIVE_NO_KEY_ONLY === "1",
    rustBin:
      process.env.RUST_CLI_BIN ??
      path.join(REPO_ROOT, "cli-rust/target/debug/primitive"),
  };
...
function isolatedEnv(tmpRoot, apiBaseUrl, apiKey) {
  ...
  for (const name of Object.keys(env).filter((item) =>
    item.startsWith("PRIMITIVE_"),
  )) {
    delete env[name];
  }
...
async function runCase(binary, env, item, state) {
  ...
  if (item.expectedFailure) {
    if (result.exitCode === 0) {
      throw new Error(
        `${item.name} unexpectedly succeeded\n${formatCapturedOutput(result)}`,
      );
    }
  ...
  const parsed =
    item.json === false ? null : parseJsonOutput(item.name, result.stdout);
...
function assertEnvelope(value, name) {
  assert(
    ...
    `${name} should print an envelope object`,
  );
    ...
    `${JSON.stringify(
      {
        ...
        oauth_client_id: "rust-live-smoke-client",
        oauth_grant_id: "rust-live-smoke-grant",
        org_id: "rust-live-smoke-org",
        org_name: null,
        refresh_token: "rust-live-smoke-refresh",
        token_type: "Bearer",
      },
      null,
      2,
...
const NO_KEY_CASES = [
  ...
  },
  {
    ...
    validate: (_value, result) => {
      assert.match(result.stdout, /Official Primitive CLI/);
  ...
  },
  {
    ...
    validate: (_value, result) => {
      assert.match(
        result.stdout,
        /complete -F _primitive_completion primitive/,
      );
  ...
  },
  {
    ...
    validate: (value) => {
      ...
      assert(
        value.length >= 100,
        "list-operations should include the generated API surface",
      );
  ...
  },
  {
    ...
    validate: (_value, result) => {
      assert.match(
        result.stderr,
        /Primitive CLI environment live-smoke is active\./,
      );
    },
  },
  {
  ...
  },
  {
    ...
    validate: (_value, result) => {
      assert.match(
        result.stderr,
        /Primitive CLI environment live-smoke removed\./,
      );
    },
  },
  {
  ...
  },
  {
    ...
    validate: (_value, result) => {
      ...
      assert.match(
        result.stdout,
        /Sends an outbound email through Primitive's outbound relay/,
      );
  ...
  return [
    {
      ...
      validate: (value) => {
        assert(
          value && typeof value === "object",
          "send should print a JSON object",
        );
        assert.equal(
          typeof value.id,
          "string",
          "send response should include id",
        );
        assert.equal(
          typeof value.from,
          "string",
          "send response should include from",
        );
        assert(
          Array.isArray(value.accepted),
          "send response should include accepted recipients",
        );
        assert(
          Array.isArray(value.rejected),
          "send response should include rejected recipients",
        );
        if ("status" in value) {
          assert.equal(
            typeof value.status,
            "string",
            "send response status should be a string",
          );
        }
...
const READ_ONLY_CASES = [
  {
    ...
    validate: (value) => {
      assert(Array.isArray(value), "list-operations should print an array");
      assert(
        value.some((operation) => operation.operationId === "getAccount"),
        "missing getAccount",
      );
    },
  },
  ...
  },
  {
    ...
    validate: (value, _result, state) => {
      ...
      assert(
        items,
        "templates:list-templates should print an array or an object with items",
      );
      state.firstTemplate =
        items.find((item) => item && typeof item.slug === "string") ?? null;
    },
  },
  {
    args: (state) => {
    ...
    validate: (value, _result, state) => {
      ...
      assert.equal(
        typeof value.title,
        "string",
        "templates:get-template should include title",
      );
    },
  },
  {
    ...
    validate: (value) => {
      ...
      assert(
        ...
        "whoami response should include account id",
      );
    },
  },
  {
    args: (state) =>
      state.includeSecretArgv
        ? ["whoami", "--api-key", state.liveApiKey, "--json"]
        : null,
    env: withoutEnvKey,
    ...
    validate: (value) => {
      ...
      assert(
        value && typeof value.id === "string",
        "flag-only whoami should include account id",
      );
    },
  },
  {
    ...
    validate: (value) => {
      assert(
        value && typeof value.id === "string",
        "saved credentials account:get-account should include account id",
      );
    },
  },
  {
    ...
    validate: (value) => {
      assert(
        value && typeof value.id === "string",
        "account:get-account should include account id",
      );
    },
  },
  {
    ...
    validate: (value) => {
      ...
      assert(
        value.data && typeof value.data.id === "string",
        "account:get-account envelope should include account id",
      );
    },
  },
  {
  ...
  },
  {
    ...
    validate: (value) => {
      assert(
        value && typeof value === "object",
        "account:get-storage-stats should print an object",
      );
    },
  },
  {
  ...
  },
  {
    ...
    validate: (value) => {
      assert(
        Array.isArray(value),
        "endpoints:list-endpoints should print an array",
      );
    },
  },
  {
    ...
    validate: (value) => {
      assert(
        Array.isArray(value),
        "filters:list-filters should print an array",
      );
    },
  },
  {
    ...
    validate: (value) => {
      assert(
        Array.isArray(value),
        "emails:search-emails should print an array",
      );
      assert(
        value.length <= 1,
        "emails:search-emails --limit 1 should return at most one item",
      );
    },
  },
  {
    ...
    validate: (value) => {
      assert(
        Array.isArray(value),
        "webhook-deliveries:list-deliveries should print an array",
      );
    },
  },
  {
    ...
    validate: (value) => {
      ...
      assert(
        ...
        "webhook-deliveries:list-deliveries envelope data should be an array",
      );
      assert(
        value.data.length <= 1,
        "webhook-deliveries:list-deliveries --limit 1 should return at most one item",
      );
    },
  },
  {
    ...
    validate: (value, _result, state) => {
      ...
      assert(
        items,
        "templates:list-templates --limit 1 should print an array or an object with items",
      );
      assert(
        items.length <= 1,
        "templates:list-templates --limit 1 should return at most one item",
      );
      state.firstTemplateForGet =
        items.find((item) => item && typeof item.slug === "string") ?? null;
    },
  },
  {
    ...
    validate: (value, _result, state) => {
      ...
      assert.equal(
        typeof value.id,
        "string",
        "templates:get-template should include id",
      );
    },
  },
  {
    ...
    validate: (value) => {
      assert(
        value && typeof value === "object",
        "inbox status should print an object",
      );
    },
  },
  {
    ...
    validate: (value) => {
      assert(
        value && typeof value === "object",
        "emails latest should print an envelope object",
      );
    },
  },
  {
  ...
  },
  {
    ...
    validate: (value) => {
      assert(
        value && typeof value === "object",
        "sending:get-send-permissions should print an object",
      );
    },
  },
  {
  ...
  },
  {
    ...
    validate: (value) => {
      assert(
        Array.isArray(value),
        "wake authorizations list should print an array",
      );
    },
  },
  {
    ...
    validate: (value) => {
      assert(
        Array.isArray(value),
        "wake dispatches list should print an array",
      );
    },
  },
  {
  ...
  },
  {
    ...
    validate: (value) => {
      assert(
        Array.isArray(value),
        "payments:list-payout-addresses should print an array",
      );
    },
  },
  {
    ...
    validate: (value) => {
      assert(
        Array.isArray(value),
        "payments:list-declined-payments should print an array",
      );
    },
  },
];
...
async function main() {
  ...
  try {
    ...
    if (!process.env.PRIMITIVE_API_KEY) {
      ...
      console.error(
        "PRIMITIVE_API_KEY is required for live Rust CLI smoke tests.",
      );
      console.error(
    ...
    assertSafeApiBaseUrl(
      options.apiBaseUrl,
      options.allowNonPrimitiveApiBaseUrl,
    );
    ...
    if (options.keepTmp) {
      ...
      if (process.env.PRIMITIVE_API_KEY && !options.noKeyOnly) {
        process.stderr.write(
          "Warning: kept temp directory may contain live credentials.\n",
        );
      }
    } else {
