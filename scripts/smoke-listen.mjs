  ...
  if (request.url === "/v1/account") data = { id: account };
    ...
    if (!entry) {
      entry = { id: randomUUID(), org_id: account, kind: "pull", name: input.name,
        enabled: true, rules: input.rules ?? {}, delivery: null };
...
let forwarder;
try {
  for (const args of [["listen", "--help"], ["listen", "init", "--help"]]) {
    const result = await run(args);
    assert.equal(result.code, 0, result.stderr);
    assert.match(result.stdout + result.stderr, /listen/);
  }
  let before = requests.length;
  const invalid = await run(["listen", "--exec", "unused", "--forward-to", "http://localhost:1234"]);
  ...
  const bare = await run(["listen", "--events", "payment.settled", "--number", "1"]);
  ...
  const resumed = await run(["listen", "--number", "1"]);
...
`);
  ...
  const executed = await run(["listen", "--subscription", "agent", "--exec", "node accept.mjs", "--number", "1"]);
  ...
  const result = await run(["listen", "--subscription", "forward", "--forward-to",
    `http://127.0.0.1:${forwarder.address().port}/webhook`, "--number", "1"]);
  assert.equal(result.code, 0, result.stderr);
