...
class CreateRouteInput:
    """ 
        Attributes:
            match_type (MatchType): How a route's pattern is matched against the recipient. exact: the full
                address. wildcard: a glob with * and ? that never crosses the @.
            pattern (str): The recipient pattern to match (interpreted per match_type).
            endpoint_id (UUID | Unset): Target an existing endpoint. Provide this OR function_id, not both.
            function_id (UUID | Unset): Mint a dedicated route-target endpoint for this Function and bind the
                route to it in one transaction. Provide this OR endpoint_id, not both.
            domain_id (None | Unset | UUID): Scope the route to one domain; null applies it org-wide.
    ...
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        match_type = MatchType(d.pop("match_type"))
