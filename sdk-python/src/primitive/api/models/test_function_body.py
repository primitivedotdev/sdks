from __future__ import annotations

from collections.abc import Mapping
from typing import Any, TypeVar, BinaryIO, TextIO, TYPE_CHECKING, Generator

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..types import UNSET, Unset







T = TypeVar("T", bound="TestFunctionBody")



@_attrs_define
class TestFunctionBody:
    """ 
        Attributes:
            local_part (str | Unset): Override the synthetic local-part. When set, the
                test email is sent to `<local_part>@<picked-domain>`
                instead of the default
                `__primitive_function_test+<random>@<picked-domain>`.
                Must start with an alphanumeric and contain only
                letters, digits, dots, plus signs, hyphens, or
                underscores; 1-64 characters total.
                 Example: summarize.
     """

    local_part: str | Unset = UNSET
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)





    def to_dict(self) -> dict[str, Any]:
        local_part = self.local_part


        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update({
        })
        if local_part is not UNSET:
            field_dict["local_part"] = local_part

        return field_dict



    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        d = dict(src_dict)
        local_part = d.pop("local_part", UNSET)

        test_function_body = cls(
            local_part=local_part,
        )


        test_function_body.additional_properties = d
        return test_function_body

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
