...
// YC cut: minimal + explanatory, paced to ~60s. Terminal + narration panel.
const SLOW = { holdScale: 3.7, leadInFrames: 165 };

export const networkSessionFrames = (): number => buildTimeline(networkSections, SLOW).total + SESSION_TAIL;
export const networkTotalFrames = (): number => INTRO_FRAMES + networkSessionFrames() + OUTRO_FRAMES;
...
export const DemoVideoNetwork: React.FC<{ format: Format }> = ({ format }) => {
  const { width, height } = useVideoConfig();
  const timeline = buildTimeline(networkSections, SLOW);
  const c = cfg[format];
    ...
    <AbsoluteFill style={{ backgroundColor: theme.pageBg }}>
      <Series>
        <Series.Sequence durationInFrames={INTRO_FRAMES}>
          <FadeIn>
            <AbsoluteFill style={{ justifyContent: "center", alignItems: "center" }}>
              <TitleCard wordmark={networkIntroTitle} primary={networkIntroTagline} scale={cardScale[format]} />
            </AbsoluteFill>
          </FadeIn>
        </Series.Sequence>

        <Series.Sequence durationInFrames={networkSessionFrames()}>
          <FadeIn>
            <AbsoluteFill style={{ padding: c.margin, flexDirection: "column", gap: c.gap }}>
              ...
              <div style={{ flex: 1, display: "flex", flexDirection: c.row ? "row" : "column", gap: c.gap, minHeight: 0 }}>
                <Terminal width={termW} height={termH} title="you@laptop — ~ — zsh" bodyPadding={Math.round(c.fontSize * 1.3)}>
                  <Session timeline={timeline} fontSize={c.fontSize} />
                </Terminal>
                <div style={{ width: panelW, height: panelH }}>
                  <ExplainerPanel ranges={timeline.ranges} fontSize={c.panelFont} sections={networkSections} />
                </div>
              </div>
            </AbsoluteFill>
          </FadeIn>
        ...
        <Series.Sequence durationInFrames={OUTRO_FRAMES}>
          <FadeIn>
            <AbsoluteFill style={{ justifyContent: "center", alignItems: "center" }}>
              <TitleCard wordmark={networkIntroTitle} primary={networkOutroInstall} secondary={networkOutroRepo} scale={cardScale[format]} mono />
            </AbsoluteFill>
          </FadeIn>
        </Series.Sequence>
      </Series>
